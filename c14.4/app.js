...
function handleOnlineMatchUpdate(payload) {
  ...
  if (
    ...
    onlineState.endedMatchNotifiedId = payload.match.id;
    const winnerName = getOnlineDisplayNameForColor(payload.match.state.winner);
    updateStatus(incomingMessage || `${winnerName} dominates the entire board.`);
    ...
    if (uiState.game.winner) {
      ...
      turnBanner.textContent =
        reconnectSeconds !== null && reconnectSeconds > 0
          ? `${currentOnlinePlayer.displayName ?? currentPlayer.name} left - waiting ${reconnectSeconds}s`
          : currentOnlinePlayer?.connected === false
          ? `${currentOnlinePlayer.displayName ?? currentPlayer.name} disconnected`
          : currentOnlinePlayer?.isBot || currentOnlinePlayer?.replacedByBot
          ? `Bot playing as ${currentOnlinePlayer.colorName ?? currentPlayer.name}`
