...
- `index.html`
  ...
  - Loads `styles.css?v=20260527-recovery` and the flattened `app.js?v=20260527-recovery`.
...
- Active-match disconnect recovery:
  - When a player disconnects from a playing online match, the server marks that player disconnected, sets `disconnectedAt`, sets `graceExpiresAt` 30 seconds in the future, and broadcasts a system message such as `[Player Name] left the match. [Player Name] has 30 seconds to rejoin.`
  - The online lobby/player list shows `Disconnected`, `Reconnecting Ns`, `Bot`, and `Forfeited` badges where applicable.
  - If the player rejoins within 30 seconds, the server clears the timeout fields and broadcasts `[Player Name] rejoined the match.`
...
- Online matchmaking/recovery panel with joined count, matched player names, status messages, Cancel/Back, Rejoin Match, and Forfeit Match actions.
