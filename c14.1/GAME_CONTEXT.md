...
- `app.js`
  ...
  - Contains online lobby/client logic for Socket.IO rooms, username persistence, reconnect identity, server-authoritative online moves, and synchronized online toss playback.
...
- `server/rooms.js`
  - Authoritative online room manager.
  - Creates short room codes, assigns colors in order, tracks host/ready/connected state, starts full rooms, validates turn ownership, applies moves, and marks disconnects.
...
- Online PvP supports 2, 3, and 4 total players chosen by the host before room creation.
...
- Online mode shows the lobby controls in setup and hides the normal setup `PLAY` button.
- The host chooses board size and total player count before pressing `Create Room`.
- AI difficulty controls are disabled in online mode.
- The server assigns colors in order: Red, Blue, Green, Amber.
- The lobby shows player color, display name, host badge, ready status, and connected/disconnected state.
- Non-host players use the `Ready` button. The host uses `Start` after the room is full and all joined players are ready.
- Online matches only start from the server after the lobby is full.
...
- Moves are sent to the server through Socket.IO. The server validates room membership, connection state, current turn, action shape, and move legality before simulating.
...
- Refresh/reconnect support is basic: each browser stores `chain-reaction-client-id`, and the last room code is stored under `chain-reaction-online-room-code`. Rejoining the same code with the same client id reconnects to the existing color slot when the room still exists.
...
- Online lobby panel with room creation/joining, room code copy, ready/start actions, status messages, and player list.
...
- Empty saved usernames are cleared and online rooms use generated color names like `Red #A42F`.
...
- Online rooms keep display names unique by adding a short suffix when needed.
...
Use separate browser tabs/windows or separate browser profiles/contexts to create and join rooms. Opening the static Python server on port `5521` keeps offline modes playable but does not provide Socket.IO for Online PvP.
...
- Online PvP automated browser verification through `http://127.0.0.1:5522/index.html`:
  - Offline Human vs Human still started on a mobile-sized viewport, toss completed, and playable cells were enabled.
  - 2-player online room was created.
  - Second browser context joined by room code.
  - Non-host player clicked Ready.
  - Host Start became enabled only after the room was full/ready.
  - Server-chosen toss completed on both clients.
  ...
  - 3-player online lobby filled successfully.
...
- Server room-store verification:
  - 4-player lobby filled successfully.
  ...
  - A near-end room state was applied and the server returned winner `red` with room status `ended`, confirming winner state is included in the authoritative room snapshot.
- Server static safety verification:
  - `/health` returned 200.
  - `/index.html` returned 200.
  - `/server/index.js` returned 404.
...
- Online rooms are in memory only. Restarting the Node server clears rooms.
- Reconnect is client-id/room-code based and intentionally basic; there is no account system or long-term persistence.
