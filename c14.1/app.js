...
const ONLINE_ROOM_STORAGE_KEY = "chain-reaction-online-room-code";
...
const onlineRoomCodeText = document.getElementById("onlineRoomCodeText");
...
function refreshUsernameControls() {
  ...
  setUsernameStatus(savedUsername ? `Saved as ${savedUsername}` : "Optional for online rooms");
}
...
function saveUsername() {
  ...
  try {
    if (result.username) {
      ...
      setUsernameStatus("Username cleared; online rooms will use color code.");
    }
  } catch {
...
function getOnlinePlayer() {
  return onlineState.room?.players.find((player) => player.clientId === getClientId()) ?? null;
}

function getOnlinePlayerForColor(colorId) {
  return onlineState.room?.players.find((player) => player.colorId === colorId) ?? null;
}
...
function renderOnlinePlayerList() {
  ...
  if (!room) {
    ...
    empty.innerHTML = `<span class="online-player-dot"></span><span class="online-player-name">No room yet</span>`;
  ...
  for (let index = 0; index < room.playerCount; index += 1) {
    ...
    if (player?.isHost) {
      ...
      host.className = "online-pill host";
...
function renderOnlineLobby() {
  ...
  const isFull = Boolean(room && room.players.length >= room.playerCount);
  const allReady = Boolean(room && room.players.every((entry) => entry.connected && entry.ready));

  if (onlineRoomCodeDisplay && onlineRoomCodeText) {
    onlineRoomCodeDisplay.hidden = !room;
    onlineRoomCodeText.textContent = room?.code ?? "----";
  }
  if (roomCodeInput && room?.code) {
    roomCodeInput.value = room.code;
  }
  if (copyRoomButton) {
    copyRoomButton.hidden = !room;
  }
  if (onlineReadyButton) {
    onlineReadyButton.hidden = !room || !isLobby || isHost;
    onlineReadyButton.textContent = player?.ready ? "Ready" : "Ready";
    onlineReadyButton.classList.toggle("is-ready", Boolean(player?.ready));
  }
  if (onlineStartButton) {
    onlineStartButton.hidden = !room || !isLobby || !isHost;
    onlineStartButton.disabled = !isFull || !allReady;
  }
  if (createRoomButton) {
    createRoomButton.disabled = Boolean(room && isLobby);
  }
  if (joinRoomButton) {
    joinRoomButton.disabled = Boolean(room && isLobby);
  }

  renderOnlinePlayerList();
  if (!onlineStatus?.classList.contains("is-error")) {
    if (!room) {
      setOnlineStatus("Create or join a room.");
    } else if (room.status === "playing" && uiState.game) {
      const current = getCurrentPlayer(uiState.game);
      const currentOnlinePlayer = getOnlinePlayerForColor(current.id);
      setOnlineStatus(
        currentOnlinePlayer?.connected === false
          ? `${currentOnlinePlayer.displayName ?? current.name} disconnected.`
          : currentOnlinePlayer?.clientId === getClientId()
          ? "Your turn"
          : `Waiting for ${currentOnlinePlayer?.displayName ?? current.name}`,
      );
    } else if (room.status === "ended") {
      setOnlineStatus("Match ended.");
    } else if (!isFull) {
      setOnlineStatus("Waiting for players.");
    } else if (!allReady) {
      setOnlineStatus("Waiting for ready players.");
    } else if (isHost) {
      setOnlineStatus("Room full. Start when ready.");
    } else {
...
function getSavedOnlineRoomCode() {
  try {
    return sanitizeRoomCodeInput(localStorage.getItem(ONLINE_ROOM_STORAGE_KEY));
  } catch {
    return "";
  }
}

function saveOnlineRoomCode(roomCode) {
  const code = sanitizeRoomCodeInput(roomCode);
  try {
    if (code) {
      localStorage.setItem(ONLINE_ROOM_STORAGE_KEY, code);
    } else {
      localStorage.removeItem(ONLINE_ROOM_STORAGE_KEY);
    }
  } catch {
    // Room-code persistence is a convenience only.
  }
}
...
function syncOnlineSetupInputs(room) {
  if (!room?.settings) {
    return;
  }
  if (BOARD_PRESETS[room.settings.preset]) {
    presetSelect.value = room.settings.preset;
  }
  playerCountSelect.value = String(room.playerCount ?? room.settings.playerCount ?? 2);
  refreshCustomSelects();
}

function applyOnlineRoom(room, { syncGame = true } = {}) {
  if (!room) {
    return;
  }
  ...
  const localPlayer = getOnlinePlayer();
  onlineState.ready = Boolean(localPlayer?.ready);
  saveOnlineRoomCode(room.code);
  syncOnlineSetupInputs(room);

  if (room.state && syncGame && !uiState.resolving && !uiState.tossing) {
    clearAiTimer();
    uiState.game = cloneGameState(room.state);
    uiState.history = [];
    uiState.mode = "online";
    renderBoard();
  }
...
async function getOnlineSocket() {
  ...
  if (!onlineState.socket) {
    ...
    onlineState.socket.on("connect", () => {
      if (isOnlineMode() && onlineState.roomCode) {
    ...
    onlineState.socket.on("online:room", (room) => {
      applyOnlineRoom(room, { syncGame: !uiState.resolving && !uiState.tossing && !onlineState.moving });
    });
...
async function updateOnlineProfileInRoom() {
  if (!onlineState.roomCode) {
    return;
  }

  const result = await emitOnline("online:joinRoom", {
    ...getOnlineIdentityPayload(),
    roomCode: onlineState.roomCode,
  });
  if (result.ok) {
    applyOnlineRoom(result.room);
    setOnlineStatus(result.reconnected ? "Reconnected." : "Player joined.");
    return;
  }
...
async function handleCreateRoom() {
  ...
  const result = await emitOnline("online:createRoom", {
    ...getOnlineIdentityPayload(),
    settings: getOnlineSettingsPayload(),
  });

  if (!result.ok) {
    setOnlineStatus(result.error ?? "Could not create room.", true);
    return;
  }
...
async function handleJoinRoom() {
  ...
  const roomCode = sanitizeRoomCodeInput(roomCodeInput?.value);
  if (!roomCode) {
  ...
  if (!result.ok) {
    setOnlineStatus(result.error ?? "Could not join room.", true);
    return;
  }
...
async function handleCopyRoomCode() {
  const roomCode = onlineState.room?.code;
  if (!roomCode) {
    return;
  }
  ...
  try {
    ...
    setOnlineStatus("Room code is ready to copy.", true);
  }
}
...
async function handleOnlineReady() {
  ...
  const player = getOnlinePlayer();
  if (!room || !player) {
    setOnlineStatus("Join a room first.", true);
    return;
  }

  const nextReady = !player.ready;
  setOnlineStatus(nextReady ? "Ready." : "Not ready.");
  const result = await emitOnline("online:setReady", {
    ...getOnlineIdentityPayload(),
    roomCode: room.code,
    ready: nextReady,
  });
  if (!result.ok) {
    setOnlineStatus(result.error ?? "Could not update ready state.", true);
    return;
  }
  applyOnlineRoom(result.room);
}
...
async function handleOnlineStart() {
  const room = onlineState.room;
  if (!room) {
    setOnlineStatus("Create a room first.", true);
    return;
  }

  setOnlineStatus("Starting match...");
  const result = await emitOnline("online:startMatch", {
    ...getOnlineIdentityPayload(),
    roomCode: room.code,
  });
  if (!result.ok) {
    setOnlineStatus(result.error ?? "Could not start match.", true);
  }
}
...
async function handleOnlineMatchStarted(payload) {
  ...
  clearAiTimer();
  onlineState.moving = false;
  applyOnlineRoom(payload.room, { syncGame: false });
  uiState.mode = "online";
...
async function sendOnlineMove(action) {
  if (onlineState.moving || uiState.resolving || uiState.tossing || !onlineState.room) {
    return;
  }

  const player = getOnlinePlayer();
  if (!player) {
    setOnlineStatus("You are not in this room.", true);
    return;
  }
...
function setModeControls() {
  ...
  aiDifficultySelect.disabled = !isAiMode || uiState.resolving || isOnline;
...
function canInteractWithCell(cell) {
  ...
  if (isOnlineMode()) {
    ...
    if (
      !player ||
      onlineState.moving ||
      onlineState.room?.status !== "playing" ||
...
async function handleNewMatch() {
  ...
  if (uiState.mode === "online") {
    ...
    setModeControls();
    renderOnlineLobby();
    setOnlineStatus(onlineState.room ? onlineState.status : "Create or join a room.");
    return;
  }
...
function handleModeChange() {
  ...
  if (uiState.mode === "online") {
    const savedRoomCode = onlineState.roomCode || getSavedOnlineRoomCode();
    if (roomCodeInput && savedRoomCode) {
...
function initializeApp() {
  ...
  restartButton?.addEventListener("click", async () => {
    ...
    if (isOnlineMode()) {
      openOptionsFromGame();
      setOnlineStatus("Online matches restart from a new room.");
      return;
    }
  ...
  roomCodeInput?.addEventListener("input", () => {
    const sanitized = sanitizeRoomCodeInput(roomCodeInput.value);
    if (roomCodeInput.value !== sanitized) {
      roomCodeInput.value = sanitized;
    }
  });
  createRoomButton?.addEventListener("click", handleCreateRoom);
  joinRoomButton?.addEventListener("click", handleJoinRoom);
  copyRoomButton?.addEventListener("click", handleCopyRoomCode);
  onlineReadyButton?.addEventListener("click", handleOnlineReady);
  onlineStartButton?.addEventListener("click", handleOnlineStart);
  ...
  getClientId();
  refreshUsernameControls();
  const savedRoomCode = getSavedOnlineRoomCode();
  if (roomCodeInput && savedRoomCode) {
    roomCodeInput.value = savedRoomCode;
  }
