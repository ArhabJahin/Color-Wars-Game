...
function renderBoard(highlights = []) {
  ...
  const temporaryLock = uiState.resolving || uiState.tossing || isAiTurn();
  const visualPlayableSet = new Set(
    uiState.game.winner
      ? []
      : getLegalActions(uiState.game, getCurrentPlayerId(uiState.game)).map(
          ({ row, col }) => `${row},${col}`,
        ),
  );
  updateBoardSizing();
  ...
  for (let row = 0; row < uiState.game.rows; row += 1) {
    for (let col = 0; col < uiState.game.cols; col += 1) {
      ...
      const owner = cell.owner
        ...
        : null;
      const visuallyPlayable = visualPlayableSet.has(`${row},${col}`);
      const isPlayable = visuallyPlayable && !temporaryLock;
      const blocked = !visuallyPlayable && !uiState.game.winner;
      const disableButton = !isPlayable && !temporaryLock;
      ...
      button.classList.toggle("interaction-locked", temporaryLock);
      ...
      button.setAttribute("aria-disabled", String(!isPlayable || temporaryLock));
      button.setAttribute(
        "aria-label",
