...
class MatchmakingStore {
  ...
  reconnectMatch({ socketId, clientId, matchId }) {
    ...
    this.setSystemMessage(match, `${player.colorName} rejoined the match.`);
    return {
      ok: true,
      match: this.snapshotMatch(match),
      matchSocketIds: this.matchSocketIds(match),
  ...
  finishMatchForLeavingPlayer(match, player, reason) {
    ...
    match.status = "ended";
    this.setSystemMessage(match, `You win. ${player.colorName} left the match.`);
    this.scheduleEndedMatchCleanup(match);
  ...
  replacePlayerWithBot(match, player, reason) {
    ...
    this.setSystemMessage(
      match,
      reason === "forfeit"
        ? `${player.colorName} left the match. ${player.colorName} has been replaced by a bot.`
        : `${player.colorName} did not return. Bot is now playing as ${player.colorName}.`,
    );
    this.maybeScheduleBotTurn(match);
    return this.snapshotMatch(match);
  }
  ...
  markDisconnected(socketId) {
    ...
    if (player) {
      ...
      this.setSystemMessage(
        match,
        `${player.colorName} left the match. ${player.colorName} has 30 seconds to rejoin.`,
      );
      this.scheduleReconnectTimeout(match, player);
      match.updatedAt = Date.now();
    }
