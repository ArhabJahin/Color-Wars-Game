...
function openOptionsFromGame() {
  ...
  if (isOnlineMode() && onlineState.match?.status === "playing") {
    setOnlineRecoveryFromMatch(onlineState.match, {
      message: "Active match paused locally. Rejoin to continue.",
    });
    setOnlineStatus("Active match paused locally. Rejoin to continue.");
    renderOnlineLobby();
  }
}
...
function getOnlineColorLabel(playerOrColorId) {
  const colorId = typeof playerOrColorId === "string"
    ? playerOrColorId
    : playerOrColorId?.colorId ?? playerOrColorId?.id;
  const poolPlayer = PLAYER_POOL.find((player) => player.id === colorId);
  if (poolPlayer) {
    return poolPlayer.name;
  }
  const colorName = typeof playerOrColorId === "object" ? playerOrColorId?.colorName : getOnlinePlayerForColor(colorId)?.colorName;
  const namedPoolPlayer = PLAYER_POOL.find((player) => player.name === colorName);
  return namedPoolPlayer?.name ?? PLAYER_POOL[0].name;
}
...
function setOnlineRecoveryFromMatch(match, { message = "" } = {}) {
  ...
  onlineState.systemMessage = message || match.systemMessage || "You have an active match.";
...
function getMatchedPlayersText(players = []) {
  ...
  return `Matched players: ${players.map((player) => `${getOnlineColorLabel(player)} joined`).join(", ")}`;
}
...
function renderOnlinePlayerList() {
  ...
  for (let index = 0; index < snapshot.playerCount; index += 1) {
    ...
    name.textContent = player ? getOnlineColorLabel(player) : `${poolPlayer.name} slot open`;
    ...
    if (player) {
      ...
      if (reconnectSeconds !== null && reconnectSeconds > 0) {
        addPill(`Reconnecting ${reconnectSeconds}s`, "warning");
...
function renderOnlineLobby() {
  ...
  const shouldShow = isOnline && (onlineState.matchmaking || onlineState.activeMatchAvailable || isNotice);
  ...
  if (onlineMatchSummary) {
    ...
    if (isRecovery && countdown !== null) {
      ...
      onlineMatchSummary.textContent = getMatchedPlayersText(snapshot?.players ?? []);
    }
  }
...
function applyOnlineMatch(match, { syncGame = true } = {}) {
  ...
  onlineState.queue = null;
  onlineState.matchmaking = false;
  onlineState.systemMessage = match.systemMessage || onlineState.systemMessage || "";
...
async function playOnlineToss(tossWinnerIndex) {
  ...
  const winnerName = getOnlineColorLabel(winner);
...
function handleOnlineMatchUpdate(payload) {
  ...
  const incomingMessage = payload.match.systemMessage || "";
  ...
  if (
    ...
    onlineState.endedMatchNotifiedId = payload.match.id;
    const winnerName = getOnlineColorLabel(payload.match.state.winner);
    updateStatus(incomingMessage || `${winnerName} wins.`);
...
async function handleOnlineMoveApplied(payload) {
  ...
  if (uiState.game.winner) {
    const winnerName = getOnlineColorLabel(uiState.game.winner);
  ...
  const currentName = getOnlineColorLabel(currentPlayer);
  updateStatus(
...
function syncStatus() {
  ...
  if (isOnlineMode() && onlineState.match) {
    if (uiState.game.winner) {
      turnBanner.textContent = `${getOnlineColorLabel(uiState.game.winner)} wins`;
    } else {
      const currentOnlinePlayer = getOnlinePlayerForColor(currentPlayer.id);
      const currentColorName = getOnlineColorLabel(currentPlayer);
      const reconnectSeconds = currentOnlinePlayer?.connected === false && currentOnlinePlayer?.graceExpiresAt
        ...
        : null;
      let turnText = `Waiting for ${currentColorName}`;
      if (reconnectSeconds !== null && reconnectSeconds > 0) {
        turnText = `${currentColorName} left - waiting ${reconnectSeconds}s`;
      } else if (currentOnlinePlayer?.connected === false) {
        turnText = `${currentColorName} disconnected`;
      } else if (currentOnlinePlayer?.isBot || currentOnlinePlayer?.replacedByBot) {
        turnText = `Bot is playing as ${currentColorName}`;
      } else if (currentOnlinePlayer?.clientId === getClientId()) {
        turnText = "Your turn";
      }
