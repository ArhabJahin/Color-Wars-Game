...
- `index.html`
  ...
  - Loads `styles.css?v=20260527-recovery` and the flattened `app.js?v=20260605-color-labels`.
...
- `server/index.js`
  ...
  - Exposes `/health`.
  - Socket.IO events currently include `online:joinQueue`, `online:cancelQueue`, `online:getActiveMatch`, `online:reconnectMatch`, `online:forfeitMatch`, `online:updateProfile`, and `online:move`.
...
- `server/matchmaking.js`
  ...
  - Assigns colors in order, broadcasts waiting queue updates, starts full matches automatically, validates turn ownership, applies moves, handles reconnect recovery, handles forfeit/timeout outcomes, and marks disconnects.
  - Uses `RECONNECT_GRACE_MS = 30_000`, `ENDED_MATCH_CLEANUP_MS = 120_000`, and `BOT_MOVE_DELAY_MS = 900`.
...
- The matchmaking screen shows `Searching for players`, the current joined count, color-only joined players such as `Red joined` and `Blue joined`, and a Cancel button.
...
- The matchmaking and match lists show color-only public player labels (`Red`, `Blue`, `Green`, `Amber`) plus badges such as `You`, `Joined`, `Connected`, `Disconnected`, `Reconnecting Ns`, `Bot`, and `Forfeited`.
- Online PvP public labels always use only the color name (`Red`, `Blue`, `Green`, or `Amber`) instead of usernames, generated fallback ids, or `color + id` text. This applies to matchmaking rows, active player cards, turn banners, toss text, winner text, reconnect/timeout messages, and bot takeover messages.
- Example online labels: `Your turn`, `Waiting for Blue`, `Bot is playing as Green`, `Blue starts`, `Red wins`, `Blue left the match. Blue has 30 seconds to rejoin.`, and `Green did not return. Bot is now playing as Green.`
...
- Online PvP now has match recovery instead of leaving users stuck on `You are already in a match`:
  - Pressing the game back button during an active online match returns to the setup/lobby screen, keeps the saved match id, and shows `Rejoin Match`, `Forfeit Match`, and `Back`.
...
- Active-match disconnect recovery:
  - When a player disconnects from a playing online match, the server marks that player disconnected, sets `disconnectedAt`, sets `graceExpiresAt` 30 seconds in the future, and broadcasts a color-only system message such as `Blue left the match. Blue has 30 seconds to rejoin.`
  - The online lobby/player list shows `Disconnected`, `Reconnecting Ns`, `Bot`, and `Forfeited` badges where applicable.
  - If the player rejoins within 30 seconds, the server clears the timeout fields and broadcasts a color-only message such as `Blue rejoined the match.`
  ...
  - In 3-player and 4-player matches, manual forfeit immediately converts that seat to a bot so the match continues.
  - Server bot turns are delayed by about 900 ms and go through the same authoritative `simulateAction()` move path as human online moves.
...
- Online matchmaking/recovery panel with joined count, color-only matched players, status messages, Cancel/Back, Rejoin Match, and Forfeit Match actions.
...
- During an active Online PvP match, returning to setup does not cancel or abandon the server match. The setup screen shows the saved active match with Rejoin Match and Forfeit Match controls.
...
- Online color-only gameplay label verification after the June 5, 2026 label update:
  ...
  - Direct `MatchmakingStore` assertions confirmed disconnect/rejoin/timeout server messages use color-only text such as `Blue left the match. Blue has 30 seconds to rejoin.` and do not include usernames or fallback ids.
- Online color-only public label verification after the broader Online PvP display update:
  ...
  - `node --check server\validation.js`
  - `node --check server\bot.js`
  - Static assertion confirmed `app.js` no longer renders `player.displayName` in the Online PvP UI, uses `getOnlineColorLabel()` for public labels, and uses color-only bot turn text such as `Bot is playing as Green`.
