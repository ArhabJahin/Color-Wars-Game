...
function renderBoard(highlights = []) {
  ...
  for (let row = 0; row < state.rows; row += 1) {
    for (let col = 0; col < state.cols; col += 1) {
      ...
      const isPlayable = canPlayCell(cell);
      const blocked = !isPlayable && !state.resolving && !state.winner;
...
function syncStatus() {
  const currentPlayer = getCurrentPlayer();
  const visualTurn = state.winner ?? currentPlayer.id;
  const accentPlayer = state.players.find((player) => player.id === visualTurn) ?? currentPlayer;
  ...
  boardShellElement.classList.toggle("is-resolving", state.resolving);
  turnBanner.dataset.turn = visualTurn;
  const nextBannerText = state.winner
    ? `${state.players.find((player) => player.id === state.winner).name} wins`
    : `${currentPlayer.name} to move`;
  if (turnBanner.textContent !== nextBannerText) {
    turnBanner.textContent = nextBannerText;
  }
  if (lastVisualTurn !== visualTurn) {
    animateTurnChange();
    lastVisualTurn = visualTurn;
  }
  const canUndo = state.history.length > 0 && !state.resolving;
...
function canPlayCell(cell) {
  const currentPlayer = getCurrentPlayer();
  if (state.winner || state.resolving) {
    return false;
  }
  ...
  return cell.owner === null && state.moveCounts[currentPlayer.id] === 0;
}
...
function animateBoardRefresh() {
  animateElement(
    boardElement,
    [
      { transform: "translateY(8px) scale(0.985)", opacity: 0.82 },
  ...
  for (const transfer of transfers) {
    ...
    const animation = particle.animate(
      [
        ...
        {
          transform: `translate(-50%, -50%) translate(${(to.x - from.x) * 0.6}px, ${(to.y - from.y) * 0.6}px) scale(1.08)`,
          opacity: 1,
          offset: 0.6,
        },
        {
          transform: `translate(-50%, -50%) translate(${to.x - from.x}px, ${to.y - from.y}px) scale(0.88)`,
          opacity: 0,
        },
      ],
      {
...
async function handleMove(row, col) {
  const cell = state.board[row][col];
  if (!canPlayCell(cell)) {
    if (state.moveCounts[getCurrentPlayer().id] === 0) {
      updateStatus("On your first turn, choose any empty cell or one already controlled by you.");
    } else {
      updateStatus("After your first turn, you can place dots only in cells already controlled by you.");
    }
    return;
  }
