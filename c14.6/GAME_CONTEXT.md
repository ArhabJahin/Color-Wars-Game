...
- `index.html`
  ...
  - Loads `styles.css?v=20260606-first-turn-unlock` and the flattened `app.js?v=20260606-first-turn-unlock`.
...
- `server/index.js`
  ...
  - Exposes `/health`.
  - Socket.IO events currently include `online:joinQueue`, `online:cancelQueue`, `online:getActiveMatch`, `online:reconnectMatch`, `online:forfeitMatch`, `online:leaveMatchView`, `online:updateProfile`, and `online:move`.
...
- `server/matchmaking.js`
  ...
  - Uses `RECONNECT_GRACE_MS = 30_000`, `ENDED_MATCH_CLEANUP_MS = 120_000`, `BOT_MOVE_DELAY_MS = 900`, `ONLINE_TURN_TIME_MS = 20_000`, and `ONLINE_TOSS_DELAY_MS = 3_000`.
...
- Active Online PvP gameplay labels use the client's local visual color names from the per-match visual map, not usernames, generated fallback ids, or raw server slot names. Examples: `Red to move`, `Purple to move`, `Cyan starts`, `Blue wins`, and `You win`.
...
- Online PvP has a server-authoritative move timer:
  ...
  - Each playing match snapshot includes `phase`, `turnStartedAt`, `turnExpiresAt`, `turnDurationMs`, `lastSkippedPlayerId`, `serverTime`, and any current `systemEvent`.
  ...
  - The server resets the timer after every legal human move, timeout-assisted bot move, or permanent abandoned-seat bot move.
...
- Bot turns do not run the normal human move timer. Permanent abandoned-seat bots use `BOT_MOVE_DELAY_MS = 900`, then the next turn receives the normal human timer if it belongs to a connected human.
...
- Floating turn badge.
- In active Online PvP, the floating turn badge can show a server-timed animated outline: white for the normal 20-second move timer and black for the reconnect grace timer when the current seat is disconnected.
...
- Timeout-assisted server bot moves show short local visual-color text such as `Purple was idle. Bot made the move.` without usernames or generated ids.
