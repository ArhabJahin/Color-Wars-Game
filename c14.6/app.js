...
function formatOnlineStartText(playerOrColorId, snapshot = onlineState.match) {
  return `${getOnlineVisualColorLabel(playerOrColorId, snapshot)} starts`;
}
...
function formatOnlineSystemEvent(match) {
  ...
  switch (event.type) {
    ...
    case "turnTimeoutBotMove":
      return `${label} was idle. Bot made the move.`;
    case "turnTimeoutNoMove":
      return `${label} was idle. No legal move was available.`;
    case "botNoMove":
...
function openOptionsFromGame() {
  ...
  const shouldLeaveOnlineMatch = isOnlineMode()
...
function clearTurnTimerUi() {
  ...
  turnBanner.classList.remove("has-turn-timer", "is-disconnect-timer");
...
function updateOnlineTurnTimerFrame() {
  ...
  turnBanner.style.setProperty("--timer-line-color", timer.mode === "disconnect" ? "#111111" : "#ffffff");
...
function syncOnlineTimerFromMatch(match) {
  ...
  if (match?.status === "playing" && onlineState.matchPhase === "playing" && !uiState.tossing) {
...
async function handleOnlineMoveApplied(payload) {
  ...
  if (payload.timeoutMove) {
    ...
    const idleMessage = `${idleLabel} was idle. Bot made the move.`;
  ...
  updateStatus(`${getOnlineVisualColorLabel(currentPlayer.id, onlineState.match)} to move.`);
}
...
async function sendOnlineMove(action) {
  ...
  if (onlineState.matchPhase !== "playing") {
    setOnlineStatus("Match is starting.");
    return;
  }
...
function syncStatus() {
  ...
  if (isOnlineMode() && onlineState.match) {
    if (uiState.game.winner) {
      ...
      clearTurnTimerUi();
    } else if (onlineState.matchPhase !== "playing" || uiState.tossing) {
      turnBanner.textContent = uiState.tossing
        ? "Tossing"
        : formatOnlineStartText(currentPlayer.id, onlineState.match);
      clearTurnTimerUi();
    } else {
      ...
      const reconnectSeconds = currentOnlinePlayer?.connected === false && currentOnlinePlayer?.graceExpiresAt
        ...
        : null;
      let turnText = `${currentColorLabel} to move`;
      if (reconnectSeconds !== null && reconnectSeconds > 0) {
